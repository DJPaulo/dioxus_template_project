...
/*
pub async fn get_questions(conn: &sqlx::AnyPool) -> Result<Vec<Question>, Error> {
    // DB logic can return many error types (rusqlite, serde_json). Use anyhow inside
    // the closure so `?` can convert any error into anyhow::Error.
    let result: anyhow::Result<Vec<Question>> = (|| -> anyhow::Result<Vec<Question>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, text, question_order, answer_type, options_json FROM questions ORDER BY question_order",
        )?;
        ...
        while let Some(row) = rows.next()? {
            ...
            let question_order: u32 = row.get(2)?;
            let answer_type_name: String = row.get(3)?;
            let options_json: Option<String> = row.get(4)?;

            let answer_type = match answer_type_name.as_str() {
                "SingleSelect" => {
                ...
                "MultipleSelect" => {
                    let options: Vec<String> =
                        serde_json::from_str(options_json.as_deref().unwrap_or("[]"))?;
                    AnswerType::MultipleSelect { options }
                }
                "Text" => AnswerType::Text,
                "Integer" => AnswerType::Integer,
                "Decimal" => AnswerType::Decimal,
                "Boolean" => AnswerType::Boolean,
                "Date" => AnswerType::Date,
                other => return Err(anyhow::anyhow!("Unknown answer type: {}", other)),
            };
    ...
    // Convert anyhow::Error -> Error (there's an impl From<anyhow::Error> for Error)
